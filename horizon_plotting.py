...
drone = Drone(x_0=x_0)
controller = MPCController(drone, horizon=1, dt=1/10, constraints=constraints, Q=Q, R=R, y_ref=y_ref)
simulation = sim.Simulation(drone, controller, dt=1/100, linear=False)
simulation.run_simulation(frames=400)
...
simulation = sim.Simulation(drone, controller, dt=1/100, linear=False)
simulation.run_simulation(frames=400)
run3_states = np.array(simulation.state_history)

labels = ["Horizon 1", "Horizon 10", "Horizon 40"]
...
plot_column_from_arrays([run1_states, run2_states, run2_states], 1, 0.01, labels=labels)